...
<script>
	...
	function confirmOrder(no) {
		if (confirm('승인하시겠습니까?')) {
			window.location='${path}/order_confirm_action.ad?order_no=' + no + '&pageNum=${paging.pageNum}';
		}	}
						...
						<tr id="olist_tr${status.index}">
							...
							<td id="btn_td">
								<c:if test="${o.order_state eq '결제대기' }">
									<input type="button" name="btnConfirm" value="승인" onclick="confirmOrder(${o.order_no})"> 
									<input type="button" name="btnCancel" value="취소" onclick="cancelOrder(${o.order_no})">
								</c:if>
							</td>
						</tr>
					</c:forEach>
