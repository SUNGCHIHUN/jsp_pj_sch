...
<script>
	...
	function deleteCustomer(id) {
		if (confirm('탈퇴처리하겠습니까?')) {
			window.location='${path}/customer_delete_action.ad?customer_id=' + id + '&pageNum=${paging.pageNum}';
		}	
	}
						...
						<tr id="clist_tr${status.index}">
							...
							<td id="btn_td">
								...
								<c:if test="${c.customer_state eq 'register'}">
									<input type="button" name="btnDelete" value="탈퇴" onclick="deleteCustomer('${c.customer_id}')">
								</c:if>
							</td>
						</tr>
					</c:forEach>
